...
public class CrudDaoImpl<T extends EntityInterface> implements CrudDao<T> {
	...
	public List<T> list(String consulta, Parameter... parametros) {
		...
		if (parametros != null)
			for (Parameter hibernateParameter : parametros) {
				query.setParameter(hibernateParameter.getName(), hibernateParameter.getParameter());
			}
	...
	public List<T> findAll(Class<T> clazz) {
		List<T> list = entityManager.createNamedQuery(clazz.getSimpleName() + ".findAll").getResultList();
		return list;
	}
